...
class Solution{
    private:
    void sus(int ind,node* root,vector<int> &vec){
        int n = vec.size();
        if(ind >= n) return;
        root->data = vec[ind];
        if (ind * 2 + 1 < n) {
            root->left = newNode(vec[ind * 2 + 1]);
            sus(ind * 2 + 1, root->left, vec);
        }
        if (ind * 2 + 2 < n) {
            root->right = newNode(vec[ind * 2 + 2]);
            sus(ind * 2 + 2, root->right, vec);
        }
    }
public:
